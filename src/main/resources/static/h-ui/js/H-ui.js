...
!function($) {
	$.Huimodalalert = function(info, speed) {
		...
		if (speed == 0 || typeof(speed) == "undefined") {
			$(document.body).append('<div id="modal-alert" class="modal modal-alert radius">' + '<div class="modal-alert-info">' + info + '</div>' + '<div class="modal-footer"> <button class="btn btn-primary radius" onClick="$.Huimodal_alert.hide()">Confirm</button></div>' + '</div>');
			$("#modal-alert").fadeIn();
		} else {
